...
type AdminAuthFlow = "signIn" | "signUp";
...
function AdminSignInForm() {
  ...
  const [activeFlow, setActiveFlow] = useState<AdminAuthFlow | null>(null);

  async function handleSubmit(flow: AdminAuthFlow) {
    setActiveFlow(flow);
    setError(null);
  ...
  const submitting = activeFlow !== null;
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
        Only the allowlisted admin can create an account or sign in to review
        pending listings. Use create account for first-time setup.
      </p>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        <button
          type="submit"
          disabled={submitting}
          className="w-full rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white"
        >
          {activeFlow === "signIn" ? "Signing in..." : "Sign in"}
        </button>
        <button
          type="button"
          disabled={submitting}
          onClick={() => void handleSubmit("signUp")}
          className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-900"
        >
          {activeFlow === "signUp" ? "Creating account..." : "Create account"}
        </button>
      </div>
    </form>
  );
